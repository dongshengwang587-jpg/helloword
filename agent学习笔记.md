...
langchian的CacheBackedEmbeddings组件封装的缓存机制，组件内部包含被包装的文本嵌入模型(underlying_embeddings)、文档嵌入数据仓库(document_embedding_store)、查询嵌入数据仓库(query_embedding_store)、每次更新的向量条数，用户

发来问题首先使用查询数据仓库将问题转换为向量->到文档嵌入数据仓库进行相似度匹配->若搜寻到则返回答案，否则让大模型解答，然后使用被包装的文本嵌入模型对问题及答案进行向量化，存储到文档嵌入数据仓库。

被包装的文本嵌入模型(underlying_embeddings)的种类：LocalFileStore、RedisStore(开发常用)


# 7. 文本嵌入模型封装心得

langchain是通过封装各文本嵌入模型接口，然后加上自建的功能，形成新的接口。
